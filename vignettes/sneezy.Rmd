...
# Exploring non-linear embeddings sneezy
...
This data arrives as a simple `tibble` that contains a key indicating 
...
A TourExperiment object can be created from a data.frame or a matrix as follows:
...
multi_te2 <- embed_nonlinear(
  ...
  num_comp = 2,
  .on = "view", 
  .engine = tsne_exact(perplexity = 10, alpha = ncol(multi_te2) / 10)
)
...
```{r}
multi_te2 <- estimate_neighbors(multi_te2, 30, .on = "view")
multi_te2 <- estimate_neighbors(multi_te2, 30, .on = "tsne_exact")

neighborSet(multi_te2, "tsne_exact")
```

You can then view the k-nearest neighbors graph overlaid on the
scatter via add_neighborhood function:

```{r}
pl +
  overlay_neighbors(sampleFactors(tsne)[,1], 
                    sampleFactors(tsne)[,2], 
                    neighborSets(multi_te2, "tsne_exact"),
                    alpha = 0.01)
