...
ggplot(
  ...
  overlay_neighbors(x = sampleFactors(pcs)[,1],
                    y = sampleFactors(pcs)[,2],
                    indices =  neighborSet(multi_te2, "pca_exact"))
```
...
embedding by clustering the complete kNN or the shared nearest neighbor (sNN) graph. 

```{r}
pl +
  overlay_knn_centroids(sampleFactors(tsne)[,1], 
                        sampleFactors(tsne)[,2], 
                        neighborSet(multi_te2, "tsne_exact"),
                        color = "darkblue") 
```
...
```{r}
set.seed(1999)
multi_te2 <- generate_bases(multi_te2, .on = "view")
